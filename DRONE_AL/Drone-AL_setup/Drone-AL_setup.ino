...
void loop(){
  ...
  if(error == 0){
   ...
   // Serial.print(F("Checking for valid receiver signals."));
    Serial.print(F("等待10秒,檢查有效的接收器信號."));
 ...
 //   Serial.println(F("Place all sticks and subtrims in the center position within 10 seconds."));
    Serial.println(F("在 10 秒內將所有搖桿和子微調設置於_中心位置。"));
    for(int i = 9;i > 0;i--){
      delay(1000);
      Serial.print(i);
      Serial.print(" ");
  ...
  if(error == 0){  
    ...
    Serial.println(F("推動roll搖桿向右, 模擬左翼向上並回到中心。"));
    //Check for throttle movement
  ...
  if(error == 0){
   ...
   // Serial.println(F("Move the pitch stick to simulate nose up and back to center"));
    Serial.println(F("推動pitch俯仰搖桿向下_並推回到中心位置"", 模擬機頭向上和回到中心。"));
    //Check for throttle movement 
  ...
  if(error == 0){
    ...
    Serial.println(F("移動偏航YAW桿向右_並推回到中心位置, 模擬機頭轉向右"));
    //Check for throttle movement
  ...
  if(error == 0){
   ...
   // Serial.println(F("When ready put the sticks back in their center positions"));
    ...
    Serial.println(F("移動pitch操縱桿向下,並回到中心點以繼續步驟"));
    check_to_continue();
  }
  ...
  if(error == 0){
    ...
    if(type == 0){
      ...
      Serial.println(F("未找到陀螺儀設備!!! (ERROR 3)"));
      error = 1;
    }
  ...
  if(error == 0){
    ...
    Serial.println(F("在 10 秒內將四軸無人機_左側抬升至45度角"));
    //Check axis movement
    check_gyro_axes(1);
    if(error == 0){
      Serial.println(F("OK!"));
      ...
      Serial.println(F("將四軸無人機放平回原位置"));
      //Serial.println(F("Move stick 'nose up' and back to center to continue"));
      Serial.println(F("移動pitch操縱桿向下,並推回到中心點以繼續"));
      check_to_continue();
     ...
     // Serial.println(F("Lift the nose of the quadcopter to a 45 degree angle within 10 seconds"));
      Serial.println(F("在 10 秒內將四軸無人機的_機頭抬高至45度角。"));
      //Check axis movement
      check_gyro_axes(2);
    }
    if(error == 0){
    ...
    //Serial.println(F("Rotate the nose of the quadcopter 45 degree to the right within 10 seconds"));
      Serial.println(F("在 10 秒內將四軸無人機的_機頭水平向右旋轉 45 度"));
      Serial.println(F(""));
      //Check axis movement
      check_gyro_axes(3);
    }
 ...
 // Serial.println(F("Move stick 'nose up' and back to center to continue"));
  ...
  if(error == 0){
    Serial.println(F("==================================================="));
    Serial.println(F("Final setup check_最終設置檢查"));
    Serial.println(F("==================================================="));
    delay(1000);
    if(receiver_check_byte == 0b00001111){
      Serial.println(F("Receiver channels ok_接收器通道_OK"));
    }
    else{
      Serial.println(F("Receiver channel verification failed!!! (ERROR 6)"));
      Serial.println(F("接收器通道驗證失敗!!! (ERROR 6)"));
      error = 1;
    }
    delay(1000);
    if(gyro_check_byte == 0b00000111){
      Serial.println(F("Gyro axes ok_陀螺儀軸沒問題"));
    }
    else{
      Serial.println(F("Gyro axes verification failed!!! (ERROR 7)"));
      Serial.println(F("陀螺儀軸驗證失敗。!!! (ERROR 7)"));
      error = 1;
    }
  }     
  ...
  if(error == 0){
    ...
    Serial.println(F(""));
    Serial.println(F("==================================================="));
    Serial.println(F("Storing EEPROM information _信息儲存到EEPROM"));
    Serial.println(F("==================================================="));
    ...
    Serial.println(F("Verify EEPROM data_驗證 EEPROM 數據。"));
    delay(1000);
    ...
    if(error == 1)
    Serial.println(F("EEPROM verification failed;EEPROM驗證失敗!!! (ERROR 5)"));   
    else Serial.println(F("Verification done _驗證完成 "));
  }
  ...
  if(error == 0){
    ...
    Serial.println(F("您現在可以上傳校準電調 ESC 程式碼。"));
  }
  else{
   Serial.println(F("The setup is aborted due to an error."));
   Serial.println(F("由於錯誤，安裝程式被中止."));
...
void check_receiver_inputs(byte movement){
  ...
  //  Serial.println(F("No stick movement detected in the last 30 seconds!!! (ERROR 2)"));
     Serial.println(F("30秒內未檢測到搖桿移動。!!! (ERROR 2)"));
  }
...
void wait_for_receiver(){
  ...
  if(zero == 0){
    ...
    Serial.println(F("未找到有效的接收器信號(ERROR 1)。"));
  }
...
void check_gyro_axes(byte movement){
  ...
  if(trigger_axis == 0){
    ...
    Serial.println(F("最後 10 秒內未檢測到角運動。!!! (ERROR 4)")); 
  }
  else
...
void intro(){
  ...
  delay(1000);
  Serial.println(F(""));
  Serial.println(F("Drone-AL Setup Program"));
  Serial.println(F(""));
  Serial.println(F("==================================================="));
  delay(1500);
  Serial.println(F(""));
