...
void setup(){
 ...
 delay(2000);
 Serial.println(" 除非您 100% 確定，否則請務必拆下螺旋槳。");
 Serial.println(" ");
 Serial.println("1.按 r 鍵讀取接收器數值測試");
 Serial.println("2.按 a 鍵顯示四軸無人機_3軸角度");
 Serial.println("3.移除USB線_連接電池,為ESC電子調速器校準");
 Serial.println("4.USB線_連接, 按1.2.3.4單按鍵_進入單馬達測試_按 5 同步測試"); 
...
void loop(){
  ...
  if(Serial.available() > 0){
    ...
    if(data == '1')Serial.println("Test motor 1 (right front CCW. 逆時針)");
    if(data == '2')Serial.println("Test motor 2 (right rear CW. 順時針)");
    if(data == '3')Serial.println("Test motor 3 (left rear CCW. 逆時針)");
    if(data == '4')Serial.println("Test motor 4 (left front CW. 順時針)");
    if(data == '5')Serial.println("Test all motors together_馬達同步測試");
